...
import { Task, GeneratedImage } from '@/types'
import {
  fetchTask,
  startTask,
  submitTask,
...
import { ArrowLeft, Sparkles, CheckCircle, Flame, Layers, Download, Trash2, Loader } from 'lucide-react'
...
export default function TaskStudio({ taskId, onBack }: TaskStudioProps) {
  ...
  const loadTaskData = useCallback(async () => {
    try {
      ...
      setTask(taskData)
      const genData = await fetchGenerations(taskId)
  ...
  const handleGenerate = async () => {
    try {
      setGenerating(true)
      const data = await triggerGeneration(taskId, {
      ...
      toast.error(ax.response?.data?.detail || 'Failed to trigger AI generation')
      setGenerating(false)
    }
  }
  ...
  const handleSubmitTask = async () => {
    try {
      ...
      const ax = err as { response?: { data?: { error?: string } } }
      toast.error(ax.response?.data?.error || 'Failed to submit task')
    } finally {
      setLoading(false)
    }
  }
  ...
  const canGenerate = task.status === 'in_progress'
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-5">
              ...
              {task.product_image_url ? (
                <div className="relative aspect-square rounded border border-gray-200 bg-gray-100 flex items-center justify-center overflow-hidden">
                  <img src={task.product_image_url} alt={task.title} className="max-w-full max-h-full object-contain" />
                </div>
              ) : (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {generations.map((gen) => (
                <div key={gen.id} className="group relative aspect-square bg-gray-100 border border-gray-200 rounded-lg overflow-hidden hover:shadow transition-all duration-300">
                  <img src={gen.image_url} alt={gen.prompt_used || 'Generation'} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex flex-col justify-between p-3">
                    ...
                    <div>
                      {gen.prompt_used && (
                        <p className="text-[10px] text-gray-200 line-clamp-2 leading-normal mb-2">{gen.prompt_used}</p>
                      )}
                      <a
                        href={gen.image_url}
                        target="_blank"
